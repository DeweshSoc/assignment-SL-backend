...
const password = (process.env.MONGODB_PASSWORD) ? encodeURIComponent(process.env.MONGODB_PASSWORD) : null;
...
async function connectToDB() {
    try {
        if(!authCredentials || !domainName || !hostName || !tld){
            throw new Error("Missing environment variables. Check environment variables")
        }
